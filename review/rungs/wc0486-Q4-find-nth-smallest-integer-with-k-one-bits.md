...
## Original text
- Tempting 2: "C(50, 25) ~ 1.26 * 10^14 combinations -- a 14-trillion element array ... 800 TB". C(50,25) = 126,410,606,437,752, i.e. 126 trillion; × 8 bytes = 1.01 PB.
- Tempting 2: "Only works for tiny k (k <= 6 or so)". C(50,6) = 15,890,700, C(50,7) = 99,884,400; no defensible threshold. Dropped.
- Tempting 1/3: "10^15 * 1ns = 10^6 seconds" — arithmetic fine, rate invented; replaced by a measured one.
- Essence: "O(50 * k) time" — true only when binomials are recomputed per step; restated.

## Verified
- Reference walk vs brute force: every x < 2^18 bucketed by popcount, 262,143 cases, 0 mismatches (`scratchpad/wc0486/checks.py`).
- Six large cases from the reference walk (e.g. n = C(50,25), k = 25 → 2^50 − 2^25) fed to every shipped language.
- `>=` walk on samples: 10 and 8. int32 Pascal table simulated: C(34,17) wraps to −1,961,361,076; (2,333,606,220, 17) → 1,118,254,868,758,793 vs 17,179,738,112.
...
- Scan for (2^40, 20): answer 9,952,741,884,412 → 2.8 h at 10^9/s.
